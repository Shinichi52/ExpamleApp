...
export function getTextOrderType(ordttype) {
    return dispatch => {
        if (ordttype === OrderTypeEnum.MARKET)
            return OrderTypeEnum.MARKET;
        if (ordttype === OrderTypeEnum.LIMIT)
            return OrderTypeEnum.LIMIT;
        if (ordttype === OrderTypeEnum.STOP)
            return OrderTypeEnum.STOP;
        if (ordttype === OrderTypeEnum.STOP_LIMIT)
...
export function requestUpdateOrder(tradingDeal) {

    return dispatch => {
        dispatchGlobalOrderListDetail = dispatch;
        Global.Operator.requestUpdateOrder(callbackNewOrderSingle, tradingDeal);
        return dispatch(loadingState());
    };
